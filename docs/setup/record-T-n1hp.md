...
| V-14 | lint `test -z "$(gofmt -l .)" && go vet ./...` (the author's command form; no Operator decision names a linter) | `.githooks/*`, `engineering-discipline.md`, `docs/ci/README.md` | `gofmt -h` (`-l` lists files that differ), `go help vet`; stack from [ADR-0010](../adr/0010-use-go-as-the-technology-stack.md) | active since `T-t8qp` (CI job `lint`, hook) | `T-nfh8` |
