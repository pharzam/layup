...
| REQ-003 | The rules and the gates of a target are protected from the agents that they govern: an agent cannot write to a rule path without an approval from an account the agents do not use. | Must | 2 | F-0003#43, F-0001#3, F-0003#64 |
...
| `REQ-002` | `layup setup` on a pilot problem statement creates a target that passes the kit's discipline tests and `layup setup verify` with zero values without a citation (`F-0003#63`); the steps follow `docs/setup/steps.tsv`, and each `human_decision` row's answers are in the target's Git before the next step. |
| `REQ-003` | On a pilot target, an audit of the history shows zero agent writes to a rule path, and a set of known-bad commits against the gates is detected in full (`F-0003#64`). |
| `REQ-004` | `layup gate` on a Go target reports one verdict per gate; a seeded layout violation reports `fail`; a gate that did not run reports `not-active`, which never counts as `pass`; the target's own baseline gates are unchanged (`F-0001#7`). |
| `REQ-005` | In a pilot task, every role transition carries a record that validates against its schema: validated transitions divided by all transitions equals 1 (`F-0003#59`). |
| `REQ-006` | In the pilot, a question that needs no human decision gets an accepted answer without a human, and the Clarification Turnaround (`F-0003#71`) is measured against its start value (`F-0004#15`); the Reversal Rate (`F-0003#72`) is measured against its start value (`F-0004#16`). |
| `REQ-007` | In the pilot, the count of PRs with a failed gate that reach human review or merge is zero (`F-0003#58`), and a repeated run of a gate on the same input gives the same verdict. |
| `REQ-008` | In the audit sample of the pilot, zero business-forking decisions were made by an agent (`F-0003#57`); the Task Intervention Rate (`F-0003#70`) is measured against its start value (`F-0004#14`). |
...
| REQ-008 | F-0003#48, F-0003#57, F-0001#12, F-0001#13, F-0001#28 | — | — | — | — |
