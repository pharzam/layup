...
| `REQ-004` | `layup gate` on a target of each pilot stack reports one verdict per gate; a seeded violation of each of the four gate kinds (layout, interface boundary, contract, test quality) reports `fail`; a gate that did not run reports `not-active`, which never counts as `pass`; the target's own baseline gates are unchanged (`F-0001#7`). |
