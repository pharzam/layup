...
2. **A handoff is one row** of `runs/<task>/handoffs.tsv`, append-only, with the header `ts, task, from_role, to_role, kind, artifact_path, artifact_sha, harness, model, verdict`. The `kind` is one of the kit's record kinds, and each kind has its transition: `plan` (planner → reviewer for the review; planner → implementer once the plan review approves it); `plan-review` (reviewer → planner); `question` (any role → operator; the route to a responsible role agent that answers without a human, REQ-006 of `PRD-0001`, phase 3, is decided by a later ADR and adds its pair then); `change`, a pull-request head (implementer → reviewer); `handover` (implementer → planner for the close-out); `review-record` (reviewer → implementer when it ends `material`, reviewer → operator when it is the last round); `fixes` (implementer → reviewer); `verdict` (planner → operator, the close-out); `decision`, an ADR or a decision note (planner → reviewer); `resource-record` (planner → operator); `escalation` (any role → idea owner, [ADR-0016](0016-escalate-by-a-deterministic-floor-and-a-declared-class.md)); `answer` (idea owner → the role that escalated; operator → the role that asked); `stall-record` (any role → examiner when the stall opens; examiner → the role that continues when the outcome is a verified recovery; examiner → operator when the package goes to the Operator; operator → the role that continues after the Operator's answer). The artifact is the kit record itself, at the commit the row names; for `change` it is the pull-request head; for `answer` it is the record that holds the answer.
3. **The engine validates each row**, `layup handoff check <checkout>`: the header and the column count; the `kind` against the list; `from_role → to_role` against the transition of that kind (the pair is checked per kind, not alone; the conditions decision 2 names — an approving plan review before the plan goes to the implementer, a review record's verdict — are read from the artifact by its own kit linter, not by the pair check); that `artifact_path` exists at `artifact_sha`; and the artifact's own kit linter where one exists (`review-record-lint` for a review record, `adr-lint` for a decision, the resource-record shape of ADR-0007). A row with a legal header but a bad transition, a missing artifact or an empty record fails: the schema does not accept everything.
