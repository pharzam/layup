...
The panel (`runs/T-7qvc/`) compared: the kit's gate roles with one TSV event table that points at kit records (B 3A, C 3-A); four delivery roles by stage (A H1); artifact owners by question kind (A H2); a Markdown record per handoff with a new linter in the kit copy (B 3B); the forge as carrier (B 3C); and C's 3-C, that the schema is a rule and the role names are content. The Operator decided (O-55, #66): the kit's gate roles — planner, implementer, reviewer, examiner, operator — with the handoff records being the plan, the review record, the verdict and the stall record, validated by the kit's linters plus a transition table.
...
2. **A handoff is one row** of `runs/<task>/handoffs.tsv`, append-only, with the header `ts, task, from_role, to_role, kind, artifact_path, artifact_sha, harness, model, verdict`. The `kind` is one of the kit's record kinds: `plan`, `plan-review`, `change` (a pull-request head), `review-record`, `fixes`, `verdict`, `escalation` ([ADR-0016](0016-escalate-by-a-deterministic-floor-and-a-declared-class.md)), `stall-record`, `decision` (an ADR or a decision note), `resource-record` (ADR-0007). The artifact is the kit record itself, at the commit the row names.
3. **The engine validates each row**, `layup handoff check <checkout>`: the header and the column count; the `kind` against the list; `from_role → to_role` against the transition table (`planner→reviewer` for a plan; `planner→implementer`; `implementer→reviewer` for a change; `reviewer→implementer` for fixes; `reviewer→operator` for a verdict; any role `→examiner` for a stall; `examiner→operator`; any role `→idea owner` for an escalation; `operator→any` for an answer); that `artifact_path` exists at `artifact_sha`; and the artifact's own kit linter where one exists (`review-record-lint` for a review record, `adr-lint` for a decision, the resource-record shape of ADR-0007). A row with a legal header but a bad transition, a missing artifact or an empty record fails: the schema does not accept everything.
