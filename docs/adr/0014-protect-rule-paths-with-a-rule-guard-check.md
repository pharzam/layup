...
3. **The guard.** On each pull-request event and each review event the job lists the changed paths. If none is a rule path, the check posts `success`. If one is, the check posts `failure` until the pull request carries **an approving review by the target's Operator identity for the checked head `X`** (ADR-0013 decision 3: the App's own row commit `Y` neither starts a run nor dismisses the approval, and an approval given on `Y` counts for `X`), or a commit signed by the Operator's key (`git verify-commit`); any other new commit dismisses the approval and re-runs the guard; an approval by the Operator starts a run that re-evaluates the guard. The Operator's identity is the approver that the setup record names for Decision Point 3, "a change to a rule path".
