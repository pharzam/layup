...
1. **Two identities.** Agents push and open pull requests as the App or as a machine account; the Operator's human account never runs an agent. The target's setup record names both (Invariant 4).
2. **The rule-path list is pinned in LAYUP**, not in the target: `.github/**`, the kit's rule documents (`docs/engineering-discipline.md`, `docs/issue-workflow.md`, `docs/guardrails.md`, `docs/adr/**`), the kit's check scripts and their fixtures, `docs/setup/branch-protection.json`, `docs/setup/steps.tsv`, the stack-gate configuration of the target, and the handoff transition table and kind list of [ADR-0015](0015-roles-are-the-kit-gate-roles-and-a-handoff-is-a-tsv-row.md). A pull request cannot shrink the list.
3. **The guard.** On each pull-request event the job lists the changed paths. If none is a rule path, the check posts `success`. If one is, the check posts `failure` until the pull request carries **an approving review by the target's Operator identity** on its current head, or a commit signed by the Operator's key (`git verify-commit`); a new commit dismisses the approval and re-runs the guard. The Operator's identity is the approver that the setup record names for Decision Point 3, "a change to a rule path".
