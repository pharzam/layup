...
| `layup stall open|examine|close TARGET` | 1 (record), 3 (procedure) | `runs/<task>/`, `handoffs.tsv` | `runs/<task>/stalls.tsv` (`open` the row; `examine` twice — the examiner's harness and model before the session, the diagnosis path after it; `close` the outcome); the examiner, not the engine, writes `stall-<id>.md` | REQ-009, REQ-010 |
