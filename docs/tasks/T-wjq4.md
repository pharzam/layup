...
| 6 | The index row, the README note removed, the onboarding and glossary rows | `sh docs/adr/adr-lint.sh`; `sh docs/links/link-lint.sh`; `sh docs/setup/setup-check.sh`; `git diff --check` | 0 each | `adr-lint: OK`; `link-lint: OK  852 links resolved`; every check `OK` (the `markers` check accepts the removed `‹slug›` note); nothing |
