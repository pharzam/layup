...
| The stopping protocol: a frozen head, the cycle cap and its non-merge verdict, materiality, and where a revealed defect goes | [Reviewing until findings decay](engineering-discipline.md#reviewing-until-findings-decay) | — | [`review-record-lint`](ci/review-record-lint.sh) | [required check](setup/branch-protection.json) (`T-afa5`) | The cap is counted from `Cycle` and the verdict matched as a string; materiality and classification stay a reviewer-s judgement |
